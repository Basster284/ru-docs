...
Чтобы установить SDK, мы используем Geode CLI [из предыдущего шага](/getting-started/geode-cli).

Чтобы скачать SDK, выполните следующую команду:
```bash
geode sdk install
```
...
Вам нужно самому переодически обновлять ваш локальный SDK, что вы можете сделать следующей командой:
```bash
geode sdk update
```
